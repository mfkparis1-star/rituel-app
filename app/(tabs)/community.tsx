...
export default function CommunityScreen() {
  ...
  const SKIN_FILTERS = [
    ...
    { id: 'dry', label: lang === 'fr' ? 'Sèche' : 'Dry' },
    { id: 'oily', label: lang === 'fr' ? 'Grasse' : 'Oily' },
    { id: 'combination', label: lang === 'fr' ? 'Mixte' : 'Combination' },
    { id: 'normal', label: lang === 'fr' ? 'Normale' : 'Normal' },
    { id: 'sensitive', label: lang === 'fr' ? 'Sensible' : 'Sensitive' },
  ];
  ...
  const sharePost = async () => {
    ...
    const { error } = await supabase.from('posts').insert({
      ...
      caption: caption.trim(), skin_type: '',
          ...
          <View key={post.id} style={styles.card}>
            <View style={styles.cardHeader}>
              ...
              <View style={styles.userInfo}>
                ...
                <Text style={styles.userSkin}>Rituel member · {new Date(post.created_at).toLocaleDateString(lang === 'fr' ? 'fr-FR' : 'en-US', { day: 'numeric', month: 'short' })}</Text>
              </View>
              ...
              <TouchableOpacity onPress={pickImage} style={styles.imagePicker}>
                {imageUri ? (
                  ...
                  <View style={styles.imagePlaceholder}>
                    ...
                    <Text style={styles.imagePlaceholderText}>{lang === 'fr' ? 'Ajouter une photo (optionnel)' : 'Add a photo (optional)'}</Text>
                  </View>
                )}
              </TouchableOpacity>
              <Text style={styles.fieldLabel}>{t.community.message}</Text>
              <TextInput style={[styles.input, { height: 100 }]} placeholder={lang === 'fr' ? 'Partagez votre expérience...' : 'Share your experience...'} placeholderTextColor={T.textSoft} value={caption} onChangeText={setCaption} multiline blurOnSubmit={false} />
...
const styles = StyleSheet.create({
  ...
  header: { paddingTop: 60, paddingBottom: 16, flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-end' },
  ...
  actions: { flexDirection: 'row', gap: 20, paddingTop: 10, borderTopWidth: 1, borderTopColor: T.border },
